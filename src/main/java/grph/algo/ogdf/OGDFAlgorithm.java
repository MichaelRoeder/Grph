		...
		if (!executable.exists())
		{
			try
			{
				...
				if (!new RegularFile(srcDirectory, "_release/libOGDF.a").exists())
				{
					...
					RegularFile archive = new RegularFile(Grph.COMPILATION_DIRECTORY, "tech:ogdf.v2012.07.zip");
					Grph.logger.logWithDate("downloading OGDF to " + Grph.COMPILATION_DIRECTORY.getPath());
					archive.setContent(NetUtilities.retrieveURLContent("http://www.ogdf.net/lib/exe/fetch.php/tech:ogdf.v2012.07.zip"));
				...
				JavaResource res = new JavaResource(OGDFAlgorithm.class, "OGDF_frontend.cpp");
				res.exportToFile(new RegularFile(Grph.COMPILATION_DIRECTORY, "OGDF_frontend.cpp"));

				// compiling the front-end
				Proces.exec("g++", Grph.COMPILATION_DIRECTORY, "-I", "OGDF", "-O3", "OGDF_frontend.cpp", "-o", "OGDF_frontend", "-LOGDF/_release",
						"-lOGDF", "-pthread");
